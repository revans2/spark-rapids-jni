        ...
        if (thread_iter != threads.end()) {
          ...
          if (current_task_id >= 0) { // Dedicated task
            ...
            if (task_budgets != nullptr && metrics != nullptr) {
              if (static_cast<size_t>(metrics->current_gpu_memory_usage) + num_bytes >
                  static_cast<size_t>(task_budgets->gpu_memory_budget_after_stealing)) {
                logger->info(
            ...
            for (long const pool_task_id : current_thread_state.pool_task_ids) {
              ...
              if (budgets_for_pool_task != nullptr && metrics_for_pool_task != nullptr) {
                if (static_cast<size_t>(metrics_for_pool_task->current_gpu_memory_usage) + num_bytes >
                    static_cast<size_t>(budgets_for_pool_task->gpu_memory_budget_after_stealing)) {
                  logger->info(
